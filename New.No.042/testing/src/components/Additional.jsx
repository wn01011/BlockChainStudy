...
export default function Additional() {
  const ref = useRef();
  // ref는 document.getElementById 와 같은 기능을 하는 Hook이다.
  ...
  // 기존에 useEffect를 사용할 경우 어떤 상태(A)의 값이 변화 시 a를 기준으로 다른 상태(b)의 값을 계산하여
  // 정의하는 경우가 있다. ex) axios를 통한 데이터 받기
  // state를 따로 정의 하지 않고 useMemo를 정의한 변수로 사용할 수 있다.
  const tValue = useMemo(() => {
    return testValue + 10;
  }, [testValue]);
  // 위의 tValue는 testValue가 변경(재정의)될 때마다 계산되어( + 10) 정의 된다.
  ...
  // useMemo와 마찬가지로 testValue의 값이 변경되었을 때만 재 정의한다.

  function testFunc1() {
    console.log(testValue + 100);
  }
  // 위와 같이 function 등등으로 함수를 선언했을 때는 리랜더링 될 때마다 함수를 다시 정의한다. useCallback을 사용시 두번째 매개 변수의 상태값이 변화하지 않으면 재정의 하지 않고 있던것을 사용한다.
  // 즉, 필요없는 코드의 실행을 막을 수 있기 때문에 최적화시 사용한다.
