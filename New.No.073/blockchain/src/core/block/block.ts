...
class BlockHeader implements IBlockHeader {
  // inmplements 는 interface를 기준으로 타입을 확인한다.
  ...
  createMerkleRoot(_data: Array<string>): TResult<string, string> {
    if (!Array.isArray(_data) || !_data.length) {
      return { isError: true, msg: "데이터가 배열이 아니거나 빈배열이다" };
    }
  ...
  getDifficulty({
    ...
    [keys: string]: number;
    //string 타입의 키에 대해서 값은 number타입을 가진다.
  }): void {
...
class Block extends BlockHeader implements IBlock {
  ...
  constructor(
    ...
    _config?: IConfig
    // 앞에 빈칸이 있을 수 없기 때문에 입력되지 않을 수 도 있는 ?는 뒤로 빠져야한다.
  ) {
    ...
    if (this.merkleRoot) {
      if (_adjustmentBlock && _config) {
        this.getDifficulty({
          ...
          DAI: _config.difficultyAdjustmentInterval,
