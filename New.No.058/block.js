...
class Block extends IBlock {
  ...
  create(_previousBlock, _adjustmentDifficulty, _adjustmentTimestamp, _data) {
    try {
      ...
      // throw 명령어를 사용해서 try문을 멈추고 catch로 입력밧을 전달한다.
  ...
  getDifficulty({
    ...
    // 높이가 난이도 조절 단위 개수 미만일 경우 최초블록에서 현재까지 난이도는 0으로 고정되어있다.
  ...
  updateBlock(_previousBlock, _adjustmentDifficulty, _adjustmentTimestamp) {
    ...
    // 현재 hash를 bit 형식으로, binary형식으로 바꾼다.
    while (!hashBinary.startsWith("0".repeat(this.difficulty))) {
      ...
      // 난이도가 2면 0이 두 개인지 확인함.
      // 블록의 난이도를 갯수만큼 "0"이 붙을 때 까지 반복
      this.nonce += 1;
      //   nonce를 증가시키고
      this.timestamp = Date.now();
      ...
      this.difficulty = this.getDifficulty(
        //난이도를 다시 구한다.
        _previousBlock.height,
        _previousBlock.timestamp,
  ...
  isValidNewBlock(_newBlock, _previousBlock) {
    ...
    if (this.createHash(_newBlock) !== _newBlock.hash) {
      ...
      return { isErorr: true, error: "Hash of block is incorrect" };
    }

    // 모두 통과해서 문제가 없으면 에러가 없다고 표시하고 블록을 반환.
    return { isErorr: false, value: _newBlock };
  }
}
